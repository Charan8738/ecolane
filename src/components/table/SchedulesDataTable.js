...
const SchedulesDataTable = ({ data, columns, pagination, actions, className, selectableRows, expandableRows }) => {
  ...
  useEffect(() => {
    ...
    if (searchText !== "") {
      defaultData = data.filter((item) => {
        return item.route_no.toLowerCase().includes(searchText.toLowerCase());
      });
...
const dataTableColumns = [
  {
    name: "Coach No",
    selector: (row) => row.coach_no,
    sortable: true,
  },
  {
    name: "Route No",
    selector: (row) => row.route_no,
    sortable: true,
    hide: 370,
  },
  {
