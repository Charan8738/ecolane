...
const TicketsTable = ({ pagination, expandableRows, actions, className, selectableRows }) => {
  ...
  const getTickets = (start, end) => {
    if (status !== "loading") dispatch(fetchTickets({ startDate: start, endDate: end, client_id: client_id }));
  };
  ...
  useEffect(() => {
    let defaultData = tickets;
    if (searchText !== "") {
      defaultData = tickets.filter((item) => {
