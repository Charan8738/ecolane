...
const MultipleListDnd = ({ dndDataSet1, dndDataSet2, selectDeviceHandler }) => {
  ...
  const handleOnDragEnd = (result) => {
    ...
    if (source.droppableId === destination.droppableId) {
      ...
      selectDeviceHandler([...result.droppable1]);
          ...
          <div ref={provided.innerRef} className="col-sm-6">
            <div className="card card-bordered p-4 h-100">
              {dndSet1.map((item, index) => (
                <Draggable draggableId={item.DeviceSerialNo} key={item.DeviceSerialNo} index={index}>
                  {(provided) => (
                    <div
