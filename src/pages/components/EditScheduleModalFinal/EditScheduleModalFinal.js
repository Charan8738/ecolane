...
const EditScheduleModalFinal = ({ onSubmitHandler, ...props }) => {
  const { errors, register, handleSubmit } = useForm();

  const [updatedDate, setUpdatedDate] = useState(new Date());
  const [formFields, setFormFields] = useState(props.isEdit ? props.formData : INITIAL_ADD_FORM);
  ...
  return (
    <div>
      <Block>
        <h3>Edit Schedule</h3>
        <br></br>
        <h5 className="text-primary">{formData[0].driver_name}</h5>
        <br></br>
...
export default EditScheduleModalFinal;
