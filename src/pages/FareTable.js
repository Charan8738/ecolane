...
const FareTable = () => {
  ...
  useEffect(() => {
    if (onSearchText !== "") {
      const filteredObject = data.filter((item) => {
        return item.Agency.toLowerCase().includes(onSearchText.toLowerCase());
      });
  ...
  useEffect(() => {
    const url =
      "https://zig-app.com/ConfigAPIV2/GetAllFareRules?Agencyname=";
                    ...
                    ? currentItems.map((item, id) => {
                        return (
                          <DataTableItem key={id}>
                            ...
                            <DataTableRow size="sm">
                              <span className="tb-product">
                                <span className="tb-sub">{item.Rules}</span>
                              </span>
                            </DataTableRow>
                            <DataTableRow size="md">
                              <span className="tb-sub">{item.TicketCost}</span>
                            </DataTableRow>
                            <DataTableRow size="md">
                              <span className="tb-sub">{item.Youths}</span>
                            </DataTableRow>
                            <DataTableRow>
                              <span className="tb-sub">{item.Senior}</span>
                            </DataTableRow>
                            <DataTableRow>
                              <span className="tb-sub">{item.ADAcertified}</span>
                            </DataTableRow>
                            <DataTableRow size="md">
                              <span className="tb-sub">{item.Medicare}</span>
                            </DataTableRow>
                            <DataTableRow size="md">
                              <span className="tb-sub">{item.Disabled}</span>
                            </DataTableRow>
                          </DataTableItem>
                        );
                      })
